# -*- coding: utf-8 -*-
# This file is an copy (with some modification) from the `rbgirshick/yacs` project (https://github.com/rbgirshick/yacs).
...
_FILE_TYPES = (io.TextIOWrapper,)

# CfgNodes can only contain a limited set of valid types
_VALID_TYPES = {tuple, list, str, int, float, bool, type(None)}

_STR_TO_LIST_SEP = re.compile(r'[,;:]')
...
class CfgNode(dict):
        ...
        for full_key, v in zip(cfg_list[0::2], cfg_list[1::2]):
            ...
            for subkey in key_list[:-1]:
                if isinstance(d, CfgNode):
                    if d.is_new_allowed() and subkey not in d:
                        d[subkey] = CfgNode(new_allowed=True)
                else:
                    msg = f"got key {'.'.join(key_prefix)!r} with value {d!r} with is not a CfgNode"
                    logger.error(msg)
                ...
                d = d[subkey]
                key_prefix.append(subkey)

            subkey = key_list[-1]
            if isinstance(d, CfgNode) and d.is_new_allowed() and subkey not in d:
                d[subkey] = None
            _assert_with_logging(
                subkey in d or (isinstance(d, CfgNode) and d.is_new_allowed()),
                "Non-existent key: {}".format(full_key))
    ...
    @classmethod
    def _load_cfg_from_file(cls, file_obj: TextIO):
...
def _check_and_coerce_cfg_value_type(replacement, original, _key, full_key):
    ...
    # Conditionally casts
    # list <-> tuple
    casts = [(tuple, list), (list, tuple), (str, list), (str, tuple)]

    for (from_type, to_type) in casts:
        converted, converted_value = conditional_cast(from_type, to_type)
        if converted:
            return converted_value
        elif isinstance(from_type, str) and isinstance(to_type, (list, tuple)):
            result = _STR_TO_LIST_SEP.split(replacement)
